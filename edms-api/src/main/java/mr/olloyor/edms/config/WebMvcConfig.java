package mr.olloyor.edms.config;
...
public class WebMvcConfig implements WebMvcConfigurer {
    ...
    public void addResourceHandlers (ResourceHandlerRegistry registry) {
        ...
        registry.addResourceHandler("/**").addResourceLocations("classpath:/static/").setCacheControl(CacheControl.maxAge(365, TimeUnit.DAYS)).resourceChain(false).addResolver(new PushStateResourceResolver());
    }
    ...
    private static class PushStateResourceResolver implements ResourceResolver {
        ...
        private Resource resolve (String requestPath, List<? extends Resource> locations) {
            ...
            if (isHandled(requestPath)) {
                return locations.stream().map(loc -> createRelative(loc, requestPath)).filter(resource -> resource != null && resource.exists()).findFirst().orElseGet(null);
            }
            return index;
        }
